...
export function workAnnotation(work: Work, t: (key: string) => string): WorkAnnotation {
  return {
    kind: t(`work.kind.${work.kind}`),
    year: work.year,
    client: work.client
      ? // "Privatkunde" is the one client value that is a generic noun and has
        // an i18n key; every other client is a proper name and stays as written.
        work.client === "Privatkunde"
...
export default function WorkAnno({
  anno,
  className,
}: {
  anno: WorkAnnotation;
  ...
  return (
    <span className={`nk-mono-caption${className ? ` ${className}` : ""}`}>
